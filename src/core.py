...
class Account:
    def __init__(self, url, name, repos, gists, followers, following) -> None:
        self.name = name
        self.url = url
        self.repos = repos
        self.gists = gists
...
def connect(username_input) -> None:
    ...
    get_information(response, github_api_url) if response.status_code == 200 else print(error)


def get_information(response, github_api_url) -> None:
    """ Capture the JSON response and get the account informations """

    print(success)

    dump = response.json()
    account = Account(github_api_url, dump['login'], dump['public_repos'], dump['public_gists'], dump['followers'], dump['following'])
